...
type ColorMessage = [text: string, color?: StyleKey, styles?: StyleKey | StyleKey[]];
function color(...messages: ColorMessage[]) {
    ...
    const formattedMessages = messages.map(([text, color, stylesArray]) => {
        ...
        } else if (stylesArray && styles[stylesArray]) {
...
export const colorError = (err: string, path: string) => {
    color(
